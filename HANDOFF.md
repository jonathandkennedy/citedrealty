...
- **10 free tools** (`tools/`) + hub `tools/index.html`: ai-visibility-checker, review-reply-generator, listing-description-generator, marketing-budget-calculator (client-side), gbp-grader (client-side, 11 weighted questions), social-hook-generator, attention-anchor-generator, commission-split-calculator (client-side; targets `real estate commission calculator` 6,600 KD0, agent-framed gross-to-net math), real-estate-name-generator (client-side; targets `real estate name generator` 140 KD0; pattern banks from the name-ideas post), open-house-sign-in-sheet (printable template with @media print CSS; targets `open house sign in sheet template` 390 KD0). The last three added 2026-07-30 — all pure client-side, no API cost.
- **83 blog posts** (`blog/`) across 9 categories (Industry News, How-To Guides, Agent Q&A, Websites, Seller Leads, Buyer Leads, AI Search, Local SEO, Strategy). See §7 for the cluster map. All recent additions carry **photo heroes** generated via `gen_blog_images_gemini.sh` (gemini-3.1-flash-image → Pillow-cropped 1200×800 JPG). The generator supports an optional per-post **`updated`** key → schema `dateModified` (keeps `datePublished` honest on a refresh).
...
- **SEO infra:** robots.txt, sitemap.xml (**109 URLs** — every indexable page; privacy/terms/method are noindex and excluded), IndexNow key file + submitter (see §10).
...
**✅ Verified pull #2, 2026-07-30 (broker/team, niches, lead-company reviews, referral economy — 6 seeds + 30 exact hydrations, all real vol/mo + KD). STATUS: 11 POSTS WRITTEN & PUBLISHED 2026-07-30** — `real-estate-referral-fees`, `luxury-real-estate-marketing`, `new-construction-real-estate-agent`, `how-do-real-estate-teams-work`, `how-to-recruit-real-estate-agents`, `what-is-a-real-estate-isa`, `buyer-agency-agreement-guide`, `real-estate-name-ideas`, `divorce-real-estate-agent`, `veteran-real-estate-agent`, `probate-real-estate-leads` — plus all three refresh wins (closing-gifts +2 FAQs; luxury-presence-alternatives gained a reviews section/FAQ + `updated`; websites-guide gained an examples FAQ + `updated`). **Queue CLEARED 2026-07-30 (second push):** after a web research pass, published `zillow-lawsuits-explained` (news; Compass dismissal 3/18/26, FTC trial 8/24/26, Taylor/Armstrong RESPA-RICO class action dismissed 7/28/26 — all sourced/dated, heavy reporting-not-legal-advice hedging), `zillow-flex` (covers the Zillow Preferred rebrand; fees reported 15–40% attributed to independent guides, confirm-with-Zillow hedging), and `pay-at-closing-real-estate-leads` (the 880 KD0 hub; program fees attributed to The Close's 2026 roundup as reported figures). Built all three tool assets: commission-split-calculator, real-estate-name-generator, open-house-sign-in-sheet (see tools inventory). Cluster details preserved below:
