...
- **43 blog posts** (`blog/`) across 9 categories (Industry News, How-To Guides, Agent Q&A, Websites, Seller Leads, Buyer Leads, AI Search, Local SEO, Strategy). Newest 3 are **question-cluster / fan-out hubs** (see §7): AI tools for agents, the new-agent 90-day playbook, and a "best lead sources according to Reddit" roundup — each ships with an **SVG placeholder hero** (`blog/img/*.svg`); real photo heroes are queued in `gen_blog_images_gemini.sh` (generate, downscale to JPG, then flip the post `img` key from `.svg` to `.jpg`).
...
- **SEO infra:** robots.txt, sitemap.xml (64 URLs — every indexable page; privacy/terms are noindex and excluded).
