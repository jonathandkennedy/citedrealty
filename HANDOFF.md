...
- **45 blog posts** (`blog/`) across 9 categories (Industry News, How-To Guides, Agent Q&A, Websites, Seller Leads, Buyer Leads, AI Search, Local SEO, Strategy). Newest additions are **question-cluster / fan-out hubs** (see §7): a Websites **guide hub** + **website-builders** spoke, AI tools for agents, the new-agent 90-day playbook, and a "best lead sources according to Reddit" roundup — all with **photo heroes** generated via `gen_blog_images_gemini.sh` (gemini-3.1-flash-image → 1200×800 JPG; the prompt map in that script now includes them).
...
- **SEO infra:** robots.txt, sitemap.xml (66 URLs — every indexable page; privacy/terms are noindex and excluded).
